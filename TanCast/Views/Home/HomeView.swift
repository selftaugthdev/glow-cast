...
struct HomeView: View {
    ...
    enum Tab { case home, forecast, timer, spf, trip }
    ...
    var body: some View {
        ZStack {
            ...
            if vm.isLoading {
                ...
                DashboardErrorView(error: loadError) {
                    Task { await vm.retry() }
                }
            } else {
                TabView(selection: $selectedTab) {
                    MainDashboardView(vm: vm, onUpgrade: { showPaywall = true })
                        .tag(Tab.home)
                    ...
                    TanTimerView(vm: vm, onUpgrade: { showPaywall = true })
                        .tag(Tab.timer)
                ...
                .environment(\.colorScheme, Color.isLightBackground(for: vm.currentUV) ? .light : .dark)
            }

            // Custom tab bar
            if !vm.isLoading && vm.loadError == nil {
                VStack {
                    Spacer()
        ...
        .onChange(of: scenePhase) { _, phase in
            if phase == .active { vm.syncRemainingTime() }
        }
...
struct DashboardErrorView: View {
    let error: HomeViewModel.LoadError
    let onRetry: () -> Void

    private var icon: String {
        switch error {
        case .locationDenied:      return "location.slash.fill"
        case .locationUnavailable: return "location.magnifyingglass"
        case .fetchFailed:         return "wifi.exclamationmark"
        }
    }
    ...
    private var title: String {
        ...
        case .locationUnavailable: return "Couldn't Find You"
        case .fetchFailed:         return "Couldn't Load UV Data"
        }
    }
    ...
    private var message: String {
        switch error {
        case .locationDenied:
            return "Tan Cast needs your location to fetch the local UV index. Enable it in Settings."
        case .locationUnavailable:
            return "We couldn't get a location fix. Move somewhere with better signal and try again."
        case .fetchFailed:
            return "The forecast didn't load. Check your connection and try again."
        }
    }
...
struct CustomTabBar: View {
    ...
    var body: some View {
        HStack(spacing: 0) {
            ...
            TabBarItem(icon: "airplane", label: "Trip", tab: .trip, selectedTab: $selectedTab)
        }
